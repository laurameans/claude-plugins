...
11. **ALWAYS upload images to R2** — never use external image URLs as featured_image. Download the image first, upload to R2 via `POST /api/images`, and use the returned `/images/...` path.
12. **ALWAYS verify affiliate links are working** before publishing. Test each Amazon ASIN URL (`https://www.amazon.com/dp/{ASIN}`) to confirm it returns a valid product page (not 404). Replace any broken ASINs with working alternatives.
